...
public class NationalCodeConstraintValidator implements ConstraintValidator<NationalCode, String> {
    ...
    private final String[] allDigitEqual = {"0000000000", "1111111111", "2222222222", "3333333333", "4444444444",
    ...
    private boolean isValidNationalCode(String nationalCode) {
        if (!StringUtils.hasText(nationalCode)) return false;

        if (nationalCode.length() != 10 || Arrays.asList(allDigitEqual).contains(nationalCode)) {
            return false;
        }
